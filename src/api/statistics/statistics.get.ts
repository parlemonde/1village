...
function getClassroomsUrl(countryId?: string, villageId?: number, phase: number = 0): string {
  let baseClassroomsURL = `/statistics/classrooms-to-monitor?phase=${phase}`;
  if (countryId) {
...
async function getClassroomsToMonitor(countryId?: string, villageId?: number, phase?: number): Promise<ClassroomToMonitor[]> {
  return (
    await axiosRequest({
      method: 'GET',
      baseURL: '/api',
      url: getClassroomsUrl(countryId, villageId, phase),
    })
  ).data;
}

export const useGetClassroomsToMonitorStats = (countryId?: string, villageId?: number, phase?: number) => {
  return useQuery(['classrooms-to_monitor-stats', countryId, villageId, phase], () => getClassroomsToMonitor(countryId, villageId, phase));
};
