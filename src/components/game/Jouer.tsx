...
import { useLatestStandard } from 'src/api/game/game.latestStandard';
...
const Jouer = ({ gameType }: GameTypePlay) => {
  //Placer en props la bonne clef et standardiser les phrases ou il y a mimic
  const { data: latestStandard } = useLatestStandard({ type: 4, subType: 2 });
  console.log(latestStandard);
  ...
  // exemple gestion phrase parametrablepar type de jeu ( a mettre dans un fichier const)
  const tpm = {
    [GameType.MIMIC]: { phrase1: 'Jouer au jeu des mimiques !' },
    [GameType.EXPRESSION]: { phrase: 'Jouer au jeu des expressions !' },
  };
  tmp[gameType][1];// exemple pour choisir le bon game type en fonction des props passer en parametre 
  // fin exemple
...
import { useType } from 'src/api/game/game.latestStandard';
import { AvatarImg } from 'src/components/Avatar';
import { Base } from 'src/components/Base';
import { Flag } from 'src/components/Flag';
import { Modal } from 'src/components/Modal';
