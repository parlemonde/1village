...
const sampleTypeToInstrument = {
  [SampleType.VOCALS]: 'accordion',
  [SampleType.HARMONIC1]: 'slide_guitar',
  [SampleType.HARMONIC2]: 'electric_guitar',
  [SampleType.MELODIC1]: 'piano',
  [SampleType.MELODIC2]: 'electric_piano',
  [SampleType.RYTHMIC1]: 'maraca',
  [SampleType.RYTHMIC2]: 'jazz_set',
  [SampleType.INTRO_CHORUS]: 'accordion',
  [SampleType.OUTRO]: 'accordion',
};
        ...
        const newData: NewAnthemData = {
          tracks: [
            ...data.verseAudios.map((sample) => ({
              ...
              sampleStartTime: 0,
              sampleVolume: 1,
              iconUrl: sampleTypeToInstrument[sample.type] || 'accordion',
            })),
    ...
    for (const { id, data } of verses) {
      if (isOldVerseData(data)) {
        const newData: NewVerseData = {
          ...
          verseLyrics: data.verseLyrics,
          chorusLyrics: data.chorus,
          verseMixUrl: data.customizedMix,
          verseMixWithIntroUrl: '',
          verseMixWithVocalsUrl: data.customizedMixWithVocals,
          verseFinalMixUrl: data.verse,
        };
        await queryRunner.query('UPDATE `activity` SET `data` = ? WHERE `id` = ?', [JSON.stringify(newData), id]);
      }
    }
    ...
    for (const { id, data } of verses) {
      if (isNewVerseData(data)) {
        const newData: OldVerseData = {
          ...
          verseLyrics: data.verseLyrics,
          chorus: data.chorusLyrics,
          verse: data.verseFinalMixUrl,
          verseStart: data.classRecordTrack.sampleStartTime,
          customizedMix: data.verseMixUrl,
          ...
          customizedMixWithVocals: data.verseMixWithVocalsUrl,
        };
        await queryRunner.query('UPDATE `activity` SET `data` = ? WHERE `id` = ?', [JSON.stringify(newData), id]);
      }
    }
